...
async function getClubs(): Promise<ClubListItem[]> {
  const response = await adminFetch('/api/admin/clubs', 'Failed to load institutions');
...
export default async function InstitucionesPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl sm:text-2xl font-bold tracking-tight">Instituciones</h1>
        <p className="text-sm sm:text-base text-muted-foreground">
          Gestión de instituciones: alta, edición y cambio de estado.
        </p>
      </div>
